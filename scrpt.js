...
let winer = false;
let move = 1;
let box = document.querySelector(".fild");
let greating = document.querySelector(".greating");
let updateButton = document.querySelector("#update");
let tur = document.querySelector(".tur");
tur.innerHTML = `<span class="animate__animated animate__flip">${fullName}, ваш ход</span>`;
...
function checkFinal() {
    if (winer || !move == 10) {congratuletion(gamer);}
    if (winer || move == 10) {gamer == 'X' ? gamer = 'O' : gamer = 'X'; congratuletion(gamer);}
    if (!winer && move !== 10) {tur.innerHTML = `<span class="animate__animated animate__fadeIn">${fullName}, ваш ход</span>`;}
    if (!winer && move == 10) {tur.textContent = `! НИЧЬЯ !`;}
};
